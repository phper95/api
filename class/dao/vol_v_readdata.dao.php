<?php
	...
	class vol_v_readdata_dao extends DaoBase{
		public $id;
		public $movie_id;
		public $vol_id;
		public $ding;
		...
		function loadFrom_MemCache_VolId($volid){
			...
			//检查vol_id_{volid}_readdata
			$key = 'vol_id_'.$this->vol_id.'_readdata';
		...
		function loadFrom_DataBase_VolId($volid){
			...
			$query = 'SELECT * FROM `vol_v_readdata` WHERE `vol_id`='.$this->vol_id .';';
		...
		function saveTo_DataBase(){
			...
			if($result){ 
				if(mysqli_num_rows($result)>0){
					//有 Update
					$query = 'UPDATE `vol_v_readdata` SET '.
					...
					//没有 Insert
					$query = 'INSERT INTO vol_v_readdata(movie_id,ding,cai,played,share,keep) VALUES ('.
								$this->movie_id.','.
								$this->ding.','.
								$this->cai.','.
								$this->played.','.
