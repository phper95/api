<?php
	...
	class zone_dao extends DaoBase{
		public $id;
		public $name;
		public $add_time;
		public $movie_id;
		...
		function loadFrom_ZoneId($connect_memcache,$connect_database,$zoneid){
			//先尝试缓存加载
			if(!$connect_memcache){
				//没有给定缓存连接
				//自己创建连接
			...
			if($this->loadFrom_MemCache_ZoneId($zoneid)){
				return true;
			}
			...
			if($this->loadFrom_DataBase_ZoneId($zoneid)){
				//数据库加载完就写入Memcache
		...
		//从缓存加载数据
		function loadFrom_MemCache_ZoneId($zoneid){
			$this->id = $zoneid;
		...
		function loadFrom_MemCache_MovieId($movieid){
			...
			//检查movie_id_{movieid}_zone
			$key = 'movie_id_'.$movieid.'_zone';
			if(($value = $this->connect_memcache-> get($key)) != FALSE) { 
				$this->id 			= $value['id'];
				$this->name 		= $value['name'];
				$this->add_time 	= $value['add_time'];
		...
		function saveTo_MemCache(){
			...
			if($this->movie_id>0){
				//movie_id_{movieid}_zone
				$key = 'movie_id_'.$this->movie_id.'_zone';
				if(($value = $this->connect_memcache-> get($key)) != FALSE) { 
					$this->connect_memcache->replace($key, $json, 0, 86400);
				}else{
					$this->connect_memcache->set($key, $json, 0, 86400);	
		...
		//从数据库加载数据
		function loadFrom_DataBase_ZoneId($zoneid){
			$this->id = $zoneid;
			...
			$query = 'SELECT * FROM `m_zone` WHERE `id`='.$this->id .';';
		...
		function loadFrom_DataBase_MovieId($movieid){
			...
			$query = 'SELECT * FROM `movie_v_zone` WHERE `movie_id`='.$movieid.';';
			$result = mysqli_query($this->connect_database,$query); 
			if($result){ 
				if(mysqli_num_rows($result)>0){
					//找到了
					$value = mysqli_fetch_assoc($result);
					$this->id 			= $value['zone_id'];
					
					return $this->loadFrom_ZoneId($this->connect_memcache,$this->connect_database,$this->id);
		...
		function saveTo_DataBase(){
			...
			if($result){ 
				if(mysqli_num_rows($result)>0){
					...
					//没有 Insert
					$query = 'INSERT INTO m_zone(name,add_time) VALUES ('.
			...
			if($this->movie_id>0){
				$query = 'SELECT * FROM `movie_v_zone` WHERE `movie_id`='.$this->movie_id .';';
				$result = mysqli_query($this->connect_database,$query); 
				if($result){ 
					if(mysqli_num_rows($result)>0){
						//有 Update
						$query = 'UPDATE `movie_v_zone` SET '.
							'`zone_id`='			.$this->id.
							' WHERE `movie_id`='	.$this->movie_id.
							';';
					}else{
						//没有 Insert
						$query = 'INSERT INTO movie_v_zone(movie_id,zone_id) VALUES ('.
									$this->movie_id.','.
									$this->id.	 
						');';
					}
		...
		*/
		function createResponse_zone($json){
			if(!$json){$json = array();}
			$json['id'] = $this->id;
			$json['zone'] = $this->name;
			return $json;
