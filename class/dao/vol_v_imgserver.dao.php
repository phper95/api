<?php
	...
	class vol_v_imgserver_dao extends DaoBase{
		public $id;
		public $movie_id;
		public $vol_id;
		public $imgserver_id;
		...
		function loadFrom_MemCache_VolId($volid){
			...
			//检查vol_id_{vol_id}_imgserver
			$key = 'vol_id_'.$this->vol_id.'_imgserver';
			if(($value = $this->connect_memcache-> get($key)) != FALSE) { 
				
				$this->id = $value['id'];
				$this->movie_id = $value['movie_id'];
				$this->vol_id = $value['vol_id'];
				$this->imgserver_id = $value['imgserver_id'];
		...
		function loadFrom_DataBase_VolId($volid){
			...
			$query = 'SELECT * FROM `vol_v_imgserver` WHERE `vol_id`='.$this->vol_id .';';
		...
		function saveTo_DataBase(){
			...
			if($result){ 
				if(mysqli_num_rows($result)>0){
					//有 Update
					$query = 'UPDATE `vol_v_imgserver` SET '.
						'`movie_id`='		.$this->movie_id.','.
						'`vol_id`='			.$this->vol_id.','.
						'`imgserver_id`='		.$this->imgserver_id.
						' WHERE `vol_id`='	.$this->vol_id.
						';';
				}else{
					//没有 Insert
					$query = 'INSERT INTO vol_v_imgserver(movie_id,vol_id,imgserver_id) VALUES ('.
								$this->movie_id.','.
								$this->vol_id.','.
								$this->imgserver_id.
					');';
		...
		function getJsonStruct(){
			...
			$json['imgserver_id']	=	$this->imgserver_id;
