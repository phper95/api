<?php
	...
	class movie_comment_dao extends DaoBase{
		public $id;
		public $user_id;
		public $movie_id;
		public $vol_id;
		...
		function loadFrom_CommentId($connect_memcache,$connect_database,$commentid){
			//先尝试缓存加载
			if(!$connect_memcache){
				//没有给定缓存连接
				//自己创建连接
			...
			if($this->loadFrom_MemCache_CommentId($commentid)){
				return true;
			}
			...
			if($this->loadFrom_DataBase_CommentId($commentid)){
				//数据库加载完就写入Memcache
		...
		//从缓存加载数据
		function loadFrom_MemCache_CommentId($commentid){
			$this->id = $commentid;
			...
			//检查movie_comment_id_{comment_id}
			$key = 'movie_comment_id_'.$this->id;
		...
		function saveTo_MemCache(){
			...
			//记录Memcache
			//movie_comment_id_{comment_id}
			$key = 'movie_comment_id_'.$this->id;
			if(($value = $this->connect_memcache-> get($key)) != FALSE) { 
				$this->connect_memcache->replace($key, $json, 0, 1800);
			}else{
				$this->connect_memcache->set($key, $json, 0, 1800);	
			}
		...
		//从数据库加载数据
		function loadFrom_DataBase_CommentId($commentid){
			$this->id = $commentid;
			...
			$query = 'SELECT * FROM `user_v_comment_movie` WHERE `id`='.$this->id .';';
		...
		*/
		function createResponse_comment($loadvol,$loaduser,$loadreply){
			$json = array();
			$json['id'] = $this->id;
			...
			if($loadreply && $this->reply_comment_id>0){
				...
				if($reply->loadFrom_CommentId($this->connect_memcache,$this->connect_database,$this->reply_comment_id)){
