<?php
	...
	class stat_user_belike_dao extends DaoBase{
		public $id;
		public $user_id;
		public $belike_times;
		...
		function loadFrom_MemCache_UserId($userid){
			...
			//检查movie_id_{movieid}_user
			$key = 'movie_id_'.$this->movie_id.'_user';
		...
		function loadFrom_DataBase_UserId($userid){
			...
			$query = 'SELECT * FROM `movie_v_user` WHERE `movie_id`='.$this->movie_id .' ORDER BY `add_time` ASC;';
		...
		function saveTo_DataBase(){
			...
			foreach($this->user_list as $user_id){
				$query = 'SELECT * FROM `movie_v_user` WHERE `movie_id`='.$this->movie_id .' AND `user_id`='.$user_id.';';
				$result = mysqli_query($this->connect_database,$query); 
				if($result){ 
					if(mysqli_num_rows($result)>0){
						//有 不操作
						...
						//没有 Insert
						$query = 'INSERT INTO movie_v_user(movie_id,user_id,add_time) VALUES ('.
										 $this->movie_id.','.
										 $user_id.','.
										 'now()'.	 
						');';
