<?php
/**
* @api {post} /gmspanel/interface/zh-cn/3.1/PCM_W_CleanWorkKey.php 删除未完成上传的图片文件
* @apiPermission pxseven
* @apiVersion 0.1.0
* @apiName CleanWorkKey
* @apiGroup Work
* @apiSampleRequest http://ser3.graphmovie.com/gmspanel/interface/zh-cn/3.1/PCM_W_CleanWorkKey.php

* @apiDescription 客户端同步作品进度到服务器
...
* @apiParam (POST) {Integer} uploadkey 需要删除的所有图片文件的作品上传标识，每次上传都会有唯一的一个uploadkey来标识
		...
		if(mysqli_num_rows($result)>0){
			...
			if($work['user_id']!=$post_userid){
				...
				if($user_result && mysqli_num_rows($user_result)>0){
					...
					$json['desc'] = "该作品为[".$user['name']."]注册作品，无法同步";
				}else{
					$json['desc'] = "该作品不是当前账户注册作品，无法同步";	
				}
				...
				//是该用户的 
				//查询是否有此uploadkey
				$query = 'SELECT * FROM `pcmaker_upload_map` WHERE `work_key`=\''.$post_workid.'\' AND `upload_key`=\''.$post_uploadkey.'\' AND `state`<>3;';
				$result = mysqli_query($connection,$query);
				if($result && mysqli_num_rows($result)>0){
					...
					*/
					...
					$post_string = 'svk='.$svk.'&workid='.$post_workid.'&uploadkey='.$post_uploadkey;
					...
					if($json_struct && isset($json_struct->status)){
						if($json_struct->status==1){
							...
							//删除所有图片中的图片记录
							$query = 'DELETE FROM `pcmaker_work_img` WHERE `work_key`=\''.$post_workid.'\' AND `upload_key`=\''.$post_uploadkey.'\';';
							mysqli_query($connection,$query);
							...
							//删除失败
							if($json_struct->error=='ErrorRequest'){
								//非法请求
								if($connection)mysqli_close($connection);
								$json['status']= 2;
								$json['usetime'] = endtime($start_time);
