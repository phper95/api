<?php
/**
* @api {post} /gmspanel/interface/zh-cn/3.1/PCM_W_ReportPercent.php 上报作品创作进度
* @apiPermission pxseven
* @apiVersion 0.1.0
* @apiName ReportPercent
* @apiGroup Work
* @apiSampleRequest http://ser3.graphmovie.com/gmspanel/interface/zh-cn/3.1/PCM_W_ReportPercent.php
...
* @apiParam (POST) {Integer} percent 用户作品进度，为整形，取值范围由0到100
...
* @apiError WorkCantModify 该作品已上线无法同步进度
...
*/
	...
	if($data && count($data)>0 && (!isset($_POST) || count($_POST)==0) && isset($data->userid) && strlen($data->userid)>0 && isset($data->workid) && strlen($data->workid)>0){
		...
		if(isset($data->percent)){
			$post_percent = htmlspecialchars(addslashes($data->percent));
		}
		...
		if(isset($_POST['percent'])){$post_percent = htmlspecialchars(addslashes($_POST['percent']));}
	...
	if($result){
		if(mysqli_num_rows($result)>0){
			...
			if($work['user_id']!=$post_userid){
				...
				//同步进度
				//如果进度是合法数据就UPDATE
				if($post_percent>=0 && $post_percent<=100){
					$query = 'UPDATE `pcmaker_work` SET '.
									'`progress`='.$post_percent.','.
									'`submit_time`=now()'.
