<?php
/**
* @api {post} /gmspanel/interface/zh-cn/3.1/PCM_S_SkinListNew.php 获取皮肤列表(新版)
* @apiPermission yongge
* @apiVersion 0.1.0
* @apiName SkinListNew
* @apiGroup Skin
* @apiSampleRequest http://ser3.graphmovie.com/gmspanel/interface/zh-cn/3.1/PCM_S_SkinListNew.php
...
* @apiSuccess (ResponseJSON) {String} skins.skin.mpic 登陆界面封面大图
* @apiSuccess (ResponseJSON) {String} skins.skin.tpic 登陆后页面顶部图
* @apiSuccess (ResponseJSON) {String} skins.skin.lpic 登陆后页面左边图
* @apiSuccess (ResponseJSON) {String} skins.skin.gpic 制作器制作页面封面图
		...
		"skins": [
					{
						...
						"title": "大鱼海棠",
						"subtitle": "推荐指数:9.0分",
						"mpic": "http://imgs4.graphmovie.com/appimage/gms_skin/dayuhaitang/mpic.jpg",
						"tpic": "http://imgs4.graphmovie.com/appimage/gms_skin/dayuhaitang/tpic.png",
						"lpic": "http://imgs4.graphmovie.com/appimage/gms_skin/dayuhaitang/lpic.png",
						"gpic": "http://imgs4.graphmovie.com/appimage/gms_skin/dayuhaitang/gpic.jpg"
					},
					{
						"id": "11",
						"title": "火星救援",
...
*/
	...
	//查询皮肤列表
	//暂时不做分页
	$query = 'SELECT * FROM `pcmaker_skin` WHERE `open`=1 AND `id`>11 ORDER BY `weight` DESC ;';
			...
			while($i<$rows_count){
				...
				if($skindata){
					$json['skins'][count($json['skins'])] = array(
						...
						"mpic"=>is_null($skindata->mpic)?'':$skindata->mpic,
						"tpic"=>is_null($skindata->tpic)?'':$skindata->mpic,
						"lpic"=>is_null($skindata->lpic)?'':$skindata->mpic,
						"gpic"=>is_null($skindata->gpic)?'':$skindata->mpic
					);
				}
				$i++;
			}
