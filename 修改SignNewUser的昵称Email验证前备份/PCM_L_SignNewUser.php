...
* @apiParam (POST) {String} [pwd=""] 用户输入的登录密码的MD5值(32位)，如果是二维码扫描登录，用户之前设定过邮箱和密码，二维码登录接口<code>QrcodeCkLogin</code>是不会返回密码的MD5的，这里直接发送上来为空字串就好了.
...
*/
	...
	if($post_userid<=0){
		...
		//成功 再查询出来
		$query = 'SELECT * FROM `client_user` WHERE `email`=\''.$post_email.'\' LIMIT 1;';
		$result = mysqli_query($connection,$query); 
		if($result){ 
			if(mysqli_num_rows($result)>0){
				//找到了
